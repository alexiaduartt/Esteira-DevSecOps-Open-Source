...
# Importa relatórios do Semgrep e do Gitleaks no DefectDojo por meio da API REST.
...
IMPORT_LOG="$RESULTS_DIR/semgrep-gitleaks-import-log.jsonl"
...
write_log() {
  ...
  printf '{"timestamp":"%s","tool":"%s","status":"%s","scan_type":"%s","file":"%s"' \
...
Uso:
  ...
  bash scripts/import-semgrep-gitleaks-defectdojo.sh --dry-run
  bash scripts/import-semgrep-gitleaks-defectdojo.sh --help
...
import_scan() {
  ...
  if [[ ! -f "$report_file" ]]; then
    log "WARN" "$tool_name" "Relatório não encontrado."
  ...
  if [[ ! -s "$report_file" ]]; then
    log "ERROR" "$tool_name" "O relatório está vazio."
    log_detail "path=$report_file"
...
log "INFO" "DefectDojo" \
  "Iniciando processamento dos relatórios Semgrep e Gitleaks."
